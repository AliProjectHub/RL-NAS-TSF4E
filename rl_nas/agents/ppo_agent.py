...
class PPOAgent:
    ...
    def get_ppo_results(self, steps):
        ...
        # Train the model
        ppo_model.learn(total_timesteps=steps, callback=rl_nas.environment.StopTrainingAfterNStepsCallback(n_steps=steps))
        ...
        ppo_actions_mae = env.envs[0].actions_mae

        # Find the best parameters
        min_mae = min(item[1] for item in ppo_actions_mae)
        best_parameters = next(item for item in ppo_actions_mae if item[1] == min_mae)
        best_parameters = best_parameters[0]

        return ppo_actions_mae, best_parameters
    ...
    def get_best_model(self, best_parameters):
        ...
        torch.save(model.state_dict(), self.path+'/'+'ppo_informer.pth')
